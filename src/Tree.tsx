...
//     centerX: number;
//     centerY: number;
//     fileHovered: string | null;
//     setFileHovered: React.Dispatch<React.SetStateAction<string | null>>;
//    }
   
//    const Tree = (props: TreeProps) => {
//     const { nodes, links, width, height, centerX, centerY, fileHovered, setFileHovered } = props;
   
//     const handleMouseOver = (event: React.MouseEvent<SVGRectElement>) => {
//       const nodeId = event.currentTarget.getAttribute('id') || '';
//       setFileHovered(nodeId);
//     };
...
//    const fileLabel = node.append("text")
//      .text(d => truncateString(d.data.name, 12))
//      .attr("transform", d => `translate(${d.r, d.r * 1.15})`)
//      .call(keepBetween(svgRef, 12, 12));
...
//   // Update nodes when simulation changes
//   useEffect(() => {
//     simulation.on("tick", () => {
...
//           <circle
//             r={node.r}
//             fill={defaultFileColors[node.data.extension]}
//             onMouseOver={handleMouseOver}
...
//             stroke="white"
//             strokeWidth={6}
//             rotate={node.depth * 1 - 0}
//           >
//             {truncateString(node.data.name, 12)}
//           </CircleText>
//         </g>
//       ))}
//     </svg>
      ...
      {nodeData.map((node: any) => (
        <g transform={`translate(${node.x}, ${node.y})`} key={node.id}>
          ...
          <CircleText
            ...
            color={defaultFileColors[node.data.extension]}
            onMouseOver={handleMouseOver}
            onMouseEnter={() => {}}
